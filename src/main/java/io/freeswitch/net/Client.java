...
public class Client {
	...
	public void connect(String host, int port, String password,
		...
		try {
			...
			// Add ESL handler
			ClientHandler handler = new ClientHandler(password,
					protocolListener);
			bootstrap.handler(new ClientInitializer(new ClientInitializer(
					handler)));
			...
			if (!future.awaitUninterruptibly(timeoutSeconds, TimeUnit.SECONDS)) {
				throw new ConnectionFailure("Timeout connecting to " + host
						+ ":" + port);
			}
			// Did not timeout
	 ...
	 * connection. See {@link Client.addEventFilter}
	 * 
